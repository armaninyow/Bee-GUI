...
public class BeeGuiPacket {
	...
	public static void sendToClient(ServerPlayerEntity player, BeehiveBlockEntity beehive, BlockState state, BlockPos pos) {
		...
		// toInitialChunkDataNbt() returns empty in 1.21.10; createNbt() has the "bees" list.
		// In 1.21.5+, all NBT keys are snake_case: "bees", "entity_data", "attributes",
		// attribute "id" and "base" (not "Name"/"Base" as in older versions).
		...
		if (beehiveNbt != null) {
			...
			for (int i = 0; i < beesList.size(); i++) {
				...
				// Custom name stored as JSON: {"text":"BeeName"} or plain "BeeName"
				...
				// Attributes use snake_case "id" and "base" in 1.21.5+

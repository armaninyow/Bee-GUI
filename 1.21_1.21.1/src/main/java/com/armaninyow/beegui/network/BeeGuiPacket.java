...
/**
 * 1.21 / 1.21.1 version of BeeGuiPacket.
 * Overrides the common/ copy because the NBT API differs from 1.21.5+:
 *   - NbtCompound getters take only a key (no default-value overload)
 *   - getList(key, type) instead of getListOrEmpty(key)
 *   - getCompound(i) returns NbtCompound directly (not Optional)
 *   - getCompound(key) instead of getCompoundOrEmpty(key)
 */
	...
	public static void sendToClient(ServerPlayerEntity player, BeehiveBlockEntity beehive, BlockState state, BlockPos pos) {
		...
		if (beehiveNbt != null) {
			...
			for (int i = 0; i < beesList.size(); i++) {
				// 1.21/1.21.1: getCompound(int) returns NbtCompound directly
				NbtCompound entry      = beesList.getCompound(i);
				// 1.21/1.21.1: getCompound(key) instead of getCompoundOrEmpty(key)
				NbtCompound entityData = entry.getCompound("entity_data");

				// 1.21/1.21.1: single-arg getters only

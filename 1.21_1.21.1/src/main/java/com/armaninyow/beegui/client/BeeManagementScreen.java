...
public class BeeManagementScreen extends Screen {
	...
	// Health bar colors: 9 steps red → green
	...
	// 1.21/1.21.1: keyPressed takes (int keyCode, int scanCode, int modifiers) — no KeyInput class
	@Override
	...
	public void render(DrawContext context, int mouseX, int mouseY, float delta) {
		...
		// Draw background (dark overlay) first, then our content on top
		...
		context.drawTexture(HONEY_BAR_BG_TEXTURE,
			honeyBarX, honeyBarY,
			0, 0,
		...
		if (progressWidth > 0) {
			context.drawTexture(HONEY_BAR_FG_TEXTURE,
				honeyBarX, honeyBarY,
				0, 0,
	...
	private void renderBeeTooltip(DrawContext context, BeeGuiPacket.BeeInfo bee, int mouseX, int mouseY) {
		...
		// 1.21/1.21.1: withShadowColor() does not exist — use withColor() only
		lines.add(Text.literal("HP: " + Math.round(bee.health()) + "/" + Math.round(bee.maxHealth()))
			.styled(s -> s.withColor(0x545454)));
